...
for k=1:N
    ...
    if dist_to_path > 3.0 && ~lmpc_active
    ...
    %% ── DETEKSI MODE: ILOS atau LMPC? ───────────────────────────
    % Cari obs dyn terdekat yang masih aktif (dalam map, belum lewat)
    min_d_dyn = inf;
    nearest_id = 0;
    for id=1:n_dyn
        d=log.dist_dyn(k,id);
        if d < min_d_dyn
            min_d_dyn=d; nearest_id=id;
        end
    end

    % Transisi mode berdasarkan jarak
  if ~lmpc_active && min_d_dyn < r_detect && ~lmpc_blocked(nearest_id)
        % Aktifkan LMPC
        lmpc_active   = true;
        lmpc_active_id= nearest_id;
        lmpc_on_time  = t;
        eInt_psi = 0;
        fprintf('  [LMPC ON ] t=%.1fs, ObsDyn%d, dist=%.2fm\n',t,nearest_id,min_d_dyn);
    elseif lmpc_active
        % Cek apakah bisa kembali ke ILOS
        d_active = log.dist_dyn(k, lmpc_active_id);
        % Obs dyn "sudah lewat" jika tidak lagi di depan USV
        obs_rel_x = obs_dyn_state(lmpc_active_id,1) - xk;
        obs_rel_y = obs_dyn_state(lmpc_active_id,2) - yk;
        obs_ahead = cos(psi)*obs_rel_x + sin(psi)*obs_rel_y; % >0 = di depan
        obs_passed = (obs_ahead < 0.5);  % obs sudah di samping/belakang USV

        cte_now = -sin(alpha)*(xk - fullPath(wpIdx,1)) + cos(alpha)*(yk - fullPath(wpIdx,2));
        cte_too_large = abs(cte_now) > 1.4;
        % Tidak boleh OFF jika obs masih sangat dekat (< r_safe*2)
        obs_still_close = (d_active < r_safe * 2.3);
        if ((d_active > r_clear && obs_passed) || d_active > r_detect*3.0 || cte_too_large ...
                || (t - lmpc_on_time) > lmpc_max_dur) ...
                && (t - lmpc_on_time) > lmpc_min_dur ...
                && ~obs_still_close
           lmpc_active    = false;
           lmpc_blocked(lmpc_active_id) = true;
           lmpc_passed(lmpc_active_id) = 0;  % reset passed flag
            lmpc_blocked_dist(lmpc_active_id) = d_active;
            lmpc_active_id = 0;
            ILOS.beta_hat  = 0;
            eInt_psi       = 0;
            eInt_u         = 0;

            % Cari titik path terdekat dari posisi USV saat ini
            [~, idx_near] = min(sqrt((fullPath(:,1)-xk).^2+(fullPath(:,2)-yk).^2));
            % Jangan set psi_d_filtered — biarkan ILOS hitung dari posisi saat ini
            % ILOS akan mengikuti path RRT* yang sudah menghindari obs statis
            wpIdx    = max(2, idx_near);
            U0_saved = min(U0_saved, 0.8);

            fprintf('  [LMPC OFF] t=%.1fs, kembali ke ILOS, dist=%.2fm\n',t,d_active);
        end
    end
    log.mode(k) = lmpc_active;
    if t > 33 && t < 42
        d2_now = log.dist_dyn(k,2);
        cte_now_dbg = -sin(alpha)*(xk-fullPath(wpIdx,1))+cos(alpha)*(yk-fullPath(wpIdx,2));
        fprintf('t=%.1f mode=%d d2=%.3f cte=%.2f\n', t, lmpc_active, d2_now, cte_now_dbg);
    end

    %% ── GUIDANCE: ILOS atau LMPC ─────────────────────────────────
    futureCurvature=max(abs(pathCurv(wpIdx:min(wpIdx+25,nWP))));

    if ~lmpc_active
        %% ════ NORMAL MODE: ILOS ════════════════════════════════
        if dist_goal<GOAL_TOL
            target_U0=0; current_Delta=1.0;
        ...
        elseif ~wpReached && dist_wp<3.0
            target_U0=0.8; current_Delta=0.6;
        elseif futureCurvature>0.15
            target_U0=0.8; current_Delta=0.8;
        elseif futureCurvature>0.08
            target_U0=0.9; current_Delta=1.2;
        else
            target_U0=U0_target; current_Delta=1.8;
        end
        ...
        % Batasi ye positif agar tidak overshoot ke zona O4
        % O4 ada di atas path — cegah USV terlalu jauh ke atas
        if ye > 2.0 && wpIdx > seg1Len
            % Di Seg2, batasi drift ke atas lebih ketat
            target_U0 = min(target_U0, 0.8);
        ...
        if abs(ye)>1.5
            ...
            psi_d_raw=wrapToPi(alpha-atan2(ye+ILOS.gamma*ILOS.beta_hat,delta_eff));
            ye_factor = min(1.0, abs(ye)/2.0);
            alpha_lpf_ilos = 0.20 + 0.26*ye_factor;
            psi_d_filtered=psi_d_filtered+alpha_lpf_ilos*wrapToPi(psi_d_raw-psi_d_filtered);
            psi_d=wrapToPi(psi_d_filtered);
    else
        %% ════ AVOIDANCE MODE: LMPC ══════════════════════════════
        % Hitung CTE untuk log (tetap update meski LMPC aktif)
        p1 =fullPath(wpIdx,:);
        ye =-sin(alpha)*(xk-p1(1))+cos(alpha)*(yk-p1(2));
        cte=ye;

        % Prediksi posisi obs dyn aktif: Constant Velocity Model
        ...
        % Jalankan LMPC solver
        [psi_d_lmpc, U0_lmpc] = lmpc_solve(...
            [xk;yk;psi], u, fullPath, wpIdx, alpha, ...
            obs_pred, obs_dyn(lmpc_active_id,3), r_safe, lmpc, lims);

        % Smooth transition: LPF pada psi_d_lmpc
        alpha_lpf=0.25;
        psi_d_filtered=psi_d_filtered+alpha_lpf*wrapToPi(psi_d_lmpc-psi_d_filtered);
        psi_d=wrapToPi(psi_d_filtered);
        target_U0 = min(U0_lmpc, 1.2);   % batasi target speed saat LMPC aktif

        log.lmpc_psi(k)=psi_d_lmpc;
